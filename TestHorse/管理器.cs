    ...
    internal class 管理器
    {
        ...
        public static void 逻辑帧()
        {
            foreach (马 当前马 in 当前比赛.参赛马)
            {
                ...
                // 更新速度
                当前马.当前速度.基础属性 += 当前马.当前加速度.最终属性;
                if (当前马.当前速度.最终属性 < 当前马.最低速度)
                {
                ...
                当前马.当前体力 -= Convert.ToSingle(20 * Math.Pow(当前马.速度.最终属性 - 当前比赛.赛道基准速度 + 12, 2) / 144) * 意志系数 * 一帧时间;
                if (当前马.当前体力 <= 0)
                {
                    当前马.当前体力 = 0;
                }
                ...
                // 回显
                工具.打印($"{当前马.名称}跑了{当前马.速度.最终属性}米，进度{当前马.已行进距离}米");
            }

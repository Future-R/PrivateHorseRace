    ...
    internal class 流程管理器
    {
        ...
        public static void 赛后结算()
        {
            ...
            for (int i = 0; i < 名次表.Count; i++)
            {
                ...
                工具.打印($"{当前马.名次,2}着【{当前马.名称}】{当前马.着差文案}，用时{Math.Round(当前马.冲线时间, 2)}");
            }
        }
        ...
        {
            当前比赛.参赛马.Sort();
            工具.打印($"{当前比赛.参赛马.First().名称}{当前比赛.参赛马.First().已行进距离}");
            foreach (马 当前马 in 当前比赛.参赛马)
            {
                // 检查弯道是否结束
                // 检查技能发动
            ...
            foreach (马 当前马 in 当前比赛.参赛马)
            {
                ...
                {
                    当前马.冲线时间 = 检定.计算冲线具体时间(当前马.用时, 当前马.当前速度.最终属性, 当前马.已行进距离);
                ...
                当前马.当前体力 -= 20 * Math.Pow(当前马.当前速度.最终属性 - 当前比赛.赛道基准速度 + 12, 2) / 144 * 意志系数 * 一帧时间;
                if (当前马.当前体力 <= 0)
                {
                    当前马.当前体力 = 0;
                }
            ...
            // 将冲线的马移出循环
            List<马> 这一帧冲线的马 = 当前比赛.参赛马.Where(x => 当前比赛.赛后马.Contains(x)).ToList();

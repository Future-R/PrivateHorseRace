...
namespace HorseRace
{
    class 主程序
    {
        ...
        public static void 进入游戏()
        {
            ...
            当前比赛.参赛马 = 按适性抽取(赛马数量);
            Console.Title = $"私密马赛 Private Derby 正在进行：{当前比赛.名称}{当前比赛.总长度}m{(当前比赛.是草地 == true ? "草地" : "泥地")}比赛";
            流程管理器.比赛初始化();
            状态显示.初始化();
            比赛播报.初始化();
            ...
            {
                打印($"{当前比赛.参赛马[i].属性优势[0]}{当前比赛.参赛马[i].属性优势[1]}{当前比赛.参赛马[i].属性优势[2]}{i + 1}号{当前比赛.参赛马[i].名称}，{当前比赛.参赛马[i].跑法}跑法，状态{干劲配置表[当前比赛.参赛马[i].干劲].名称}");
            }
            Console.ReadKey();
            流程管理器.开跑();
            流程管理器.赛后结算();
